# Script path: automated_script_datasets_final_ with_XGBoost_SERA.py
...
# This script incorpoares an additional regression model, XGBoost-SERA, which is an XGBoost regressor with a custom objective function (based on the SERA metric) that incorporates the relevance function.
...
## XGB-SERA REGRESSION MODEL DEFINITION ## ---------------------------------------------------------------------------------

# --- Add phi and sigma logic ---
def compute_sigma(phi, T=1000):
    """ Computes the sigma values based on the phi values.
    Parameters:
    - phi: array-like, the phi values computed from the relevance function
    - T: int, the number of thresholds to consider for sigma computation
    Returns:
    - sigma: array-like, the computed sigma values
    """

    n = len(phi)
    sigma = np.full(n, 0.5)
    steps = np.linspace(0, 1, T + 1)[1:-1]
    for threshold in steps:
        sigma += (phi >= threshold).astype(float)
    sigma += 0.5 * (phi == 1)
    sigma /= T
    return sigma

# --- Define custom objective function for XGBoost ---
def sera_objective(y_pred, dtrain):
    """ Custom objective function for XGBoost that incorporates the SERA metric.
    Parameters:
    - y_pred: array-like, predicted values
    - dtrain: xgboost.DMatrix, training data
    Returns:
    ...
    ctrl_pts  = relevance_function_ctrl_pts.phi_ctrl_pts(y_true)
    phi = relevance_function.phi(pd.Series(y_true), ctrl_pts)
    sigma = compute_sigma(phi)
    grad = 2 * sigma * (y_pred - y_true)
    hess = 2 * sigma
    return grad, hess


# --- Define XGBSERA class ---
class XGBSERA(BaseEstimator, RegressorMixin):
    def __init__(self, max_depth=3, eta=0.1, n_estimators=100, nthread=-1):
        """ Initializes the XGB-SERA model with specified hyperparameters.
        Parameters:
        - max_depth: int, maximum depth of the trees
        - eta: float, learning rate
        - n_estimators: int, number of boosting rounds
        - nthread: int, number of threads to use for training
        """
    ...
    def fit(self, X, y):
        """ Fits the XGB-SERA model to the training data.
        Parameters:
    ...
    def predict(self, X):
        """ Predicts target values for the given features using the trained XGB-SERA model.
        Parameters:
        - X: array-like, shape (n_samples, n_features), features for prediction
        Returns:
        - predictions: array-like, shape (n_samples,), predicted target values
        """
...
def get_stratified_kfold_splits(X, y, n_splits=5, n_repeats=2, random_state=4):
    ...
    y_binned = pd.qcut(y, q=10, duplicates='drop')  # or use  KBinsDiscretizer
